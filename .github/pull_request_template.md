## What

<!-- What does this change? -->

## Contribution type

- [ ] proposal or Candidate
- [ ] evidence, counterexample, or independent review
- [ ] adopter Profile or implementation
- [ ] decision, registry, schema, verifier, or release surface
- [ ] governance, documentation, or maintenance

## Claim and evidence

<!-- State the applicability boundary, evidence, counterevidence/falsifiers, and non-claims. -->

## Interests and review

<!-- Disclose material organizational, financial, product, or implementation interests. Write "none known" when none apply. -->

- [ ] A substantive change has a reviewer other than its author
- [ ] Activation or Foundation Revision has an independent reviewer

## Compatibility and registry agreement

- [ ] `node scripts/check.mjs` passes
- [ ] Applicability, compatibility, migration, and supersession impact are explicit
- [ ] Published coordinates remain immutable
- [ ] After Foundation Freeze, decision texts remain append-only (supersession over rewrite)

## Version impact (per KFD-1)

- [ ] patch (content operation) / minor (registry schema additive) / major (machine surface breakage)
