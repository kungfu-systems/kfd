...
| KFD-4 | active | `decisions/KFD-4.md` | `schemas/kfd-4/*` | fixed I8-I12 positive and negative vectors | Rust/WASM | complete | independent cross-domain replay corpus |
...
| KFD-8 | draft | `decisions/KFD-8.md` | `schemas/kfd-8/atlas-coordinate.schema.json` | fixed current, degraded, conflict, rewrite, cut, state, and inference vectors | Rust/WASM | partial | independent adopter evidence before any activation consideration |
...
`npm run check:perspective-conformance` verifies the KFD-4 and KFD-8 fixed
vectors offline and requires native/WASM reports to match byte for byte while
retaining KFD-4 active and KFD-8 draft status boundaries.
