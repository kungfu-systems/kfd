...
| Delegated Work challenge report | Exact package, release anchor, profile manifest, six-pair suite, every fixture, projection, projected state, expected outcome, optional adapter, transcript, result, and report roots | Projection and result roots reuse KFD canonical JSON; packaged and adapter artifacts use exact SHA-256 bytes | One known pair or all six in canonical order, allowlisted projection paths, A-liveness/B-safety polarity, explicit adapter authority and replacement continuity, fixed non-certifying scope | None in the experimental profile | The single host-side Node verifier proves evidence closure and fixture-relative information findings; it does not certify a product mapping, policy, enforcement, or vendor architecture |
...
| GAP-KFD-DW-1 | KFD Delegated Work challenge | The Lab executes a local adapter process and binds installed package resources, while generic Rust/WASM verification consumes process-free self-contained bundle kinds | Duplicating only the collision or transcript checks would create a second, partial semantic authority without equivalent artifact and process boundaries | Keep the complete v1 challenge report semantics in one packaged host verifier; regression-test native/WASM profiles unchanged, and move only through a future versioned bundle kind with complete byte parity |
